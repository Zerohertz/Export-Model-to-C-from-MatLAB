...
class MWMkldnnUtils {
  ...
  public:
    ...
    static void configureReorder(MWCNNLayerImpl*,
                                 ...
                                 int index = 0);
    static void TargetImplconfigureReorder(MWTargetNetworkImpl* targetImpl,
                                           MWTensorBase* srcTensor,
                                           mkldnn::memory::format_tag dstDataFormat);

    static std::shared_ptr<mkldnn::memory> getLayerMemoryPrimitive(MWTensorBase* aTensor);
    static format_type getformatType(mkldnn::memory::desc inputdesc, MWTensorBase* srcTensor);

    static bool checkformatType(mkldnn::memory::desc inputdesc,
                                MWTensorBase* srcTensor,
                                mkldnn::memory::format_tag dstDataFormat);
    static bool isReorderNeeded(format_type formatTag);
    static void reorderInputsTo(MWCNNLayerImpl*, format_type = format_type::OPTIMAL_FORMAT);
    static mkldnn::memory::format_tag getMKLDNNFormatTag(format_type formatTag);
};
